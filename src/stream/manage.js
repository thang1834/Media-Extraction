...
import { STREAM, CACHE_TTL } from "../util/constants.js";
...
const STREAM_CLEANUP_INTERVAL = STREAM.CLEANUP_INTERVAL;
const STREAM_TTL = STREAM.TTL;

/**
 * Cleanup expired internal streams
 * 
 * Removes streams that have exceeded their TTL to prevent memory leaks.
...
function cleanupExpiredStreams() {
    ...
    for (const [id, stream] of internalStreamCache.entries()) {
        if (stream.createdAt && stream.createdAt + STREAM_TTL < now) {
            closeRequest(stream.controller);
    ...
    if (cleaned > 0) {
        logger.debug({ cleaned }, 'Cleaned up expired internal streams');
    }
}

// Start cleanup interval
setInterval(cleanupExpiredStreams, STREAM_CLEANUP_INTERVAL);

/**
 * Create a stream tunnel URL
 * 
 * Encrypts stream data and stores it in cache, then returns a secure tunnel URL.
 * The stream data is stored asynchronously to prevent race conditions.
 * 
 * @param {Object} obj - Stream configuration object
 * @returns {Promise<string>} Tunnel URL string
 */
export async function createStream(obj) {
    ...
    // Await stream cache storage to prevent race conditions
    // This ensures the stream is stored before the URL is returned
    // If storage fails, we still return the URL but log the error
    try {
        ...
        // Log error but don't fail - the URL can still be used
        // verifyStream will handle missing cache gracefully
        logError(error, { streamID }, 'Failed to store stream');
    }
...
/**
 * Create multiple proxy tunnel URLs
 * 
 * Creates tunnel URLs for video/audio URLs, subtitles, and cover images.
 * 
 * @param {Object} info - Stream information object
 * @returns {Promise<string[]>} Array of tunnel URL strings
 */
export async function createProxyTunnels(info) {
    const proxyTunnels = [];
    ...
    // Create tunnels for all URLs in parallel
    ...
    // Add subtitle tunnel if available
    if (info.subtitles) {
        urlPromises.push(
            createStream({
                ...tunnelTemplate,
    ...
    // Wait for all tunnels to be created
...
export function createInternalStream(url, obj = {}, isSubtitles) {
    ...
    // Store stream info with creation timestamp for cleanup
...
export async function verifyStream(id, hmac, exp, secret, iv) {
    try {
        ...
        if (error.name === 'SyntaxError' || error.name === 'TypeError') {
            return { status: 400 }; // Invalid encrypted data format
        }
