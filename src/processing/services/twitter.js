...
export default async function({ id, index, toGif, dispatcher, alwaysProxy, subtitleLang }) {
    ...
    const proxyMedia = async (url, filename) => await createStream({
        service: "twitter",
        type: "proxy",
        url, filename,
    });
    ...
    switch (media?.length) {
        ...
        default:
            const proxyThumb = async (url, i) =>
                await proxyMedia(url, `twitter_${id}_${i + 1}.${getFileExt(url)}`);

            // Process all media items in parallel
            const pickerPromises = media.map(async (content, i) => {
                if (content.type === "photo") {
                    let url = `${content.media_url_https}?name=4096x4096`;
                    let proxiedImage = await proxyThumb(url, i);
                ...
                if (needsFixing(content) || shouldRenderGif) {
                    ...
                    url = await proxyMedia(url, videoFilename);
                }

                return {
                    type,
                    url,
                    thumb: await proxyThumb(content.media_url_https, i),
                }
            });
