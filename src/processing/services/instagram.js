...
export default function instagram(obj) {
    ...
    async function extractOldPost(data, id, alwaysProxy) {
        ...
        if (sidecar) {
            ...
            const pickerPromises = sidecar.edges.filter(e => e.node?.display_url)
                .map(async (e, i) => {
                    ...
                    // Thumbnails always need to be proxied
                    const thumb = await createStream({
                        service: "instagram",
                        type: "proxy",
                        url: e.node?.display_url,
            ...
            const picker = await Promise.all(pickerPromises);
    ...
    async function extractNewPost(data, id, alwaysProxy) {
        const carousel = data.carousel_media;
        if (carousel) {
            // Process all items in parallel
            const pickerPromises = carousel.filter(e => e?.image_versions2)
                .map(async (e, i) => {
    ...
    async function getPost(id, alwaysProxy) {
        ...
        if (data?.gql_data) {
            result = await extractOldPost(data, id, alwaysProxy)
        } else {
            result = await extractNewPost(data, id, alwaysProxy)
        }
