...
export const loadEnvs = (env = process.env) => {
    ...
    return {
        ...
        requestTimeout: (env.REQUEST_TIMEOUT && parseInt(env.REQUEST_TIMEOUT)) || 30000, // 30 seconds default
...
export const validateEnvs = async (env) => {
    ...
    if (env.instanceCount > 1 && !env.redisURL) {
        ...
        }, 'API_INSTANCE_COUNT is not supported in your environment. Node.js version must be >= 23.1.0, and you must be running a recent enough version of linux (or other OS that supports it). For more info, see reusePort option on https://nodejs.org/api/net.html#serverlistenoptions-callback');
    ...
    if (env.customInnertubeClient && !Constants.SUPPORTED_CLIENTS.includes(env.customInnertubeClient)) {
        logger.error({
            provided: env.customInnertubeClient,
            supported: Constants.SUPPORTED_CLIENTS,
    ...
    if (env.forceLocalProcessing && !forceLocalProcessingOptions.includes(env.forceLocalProcessing)) {
        logger.error({
            provided: env.forceLocalProcessing,
            supported: forceLocalProcessingOptions,
        }, 'FORCE_LOCAL_PROCESSING is invalid');
    ...
    if (env.enableDeprecatedYoutubeHls && !youtubeHlsOptions.includes(env.enableDeprecatedYoutubeHls)) {
        logger.error({
            provided: env.enableDeprecatedYoutubeHls,
            supported: youtubeHlsOptions,
        }, 'ENABLE_DEPRECATED_YOUTUBE_HLS is invalid');
    ...
    if (env.externalProxy && !loggedProxyWarning) {
        logger.warn({
            deprecated: 'API_EXTERNAL_PROXY',
            replacement: 'HTTP_PROXY or HTTPS_PROXY',
        }, 'API_EXTERNAL_PROXY is deprecated and will be removed in a future release. Use HTTP_PROXY or HTTPS_PROXY instead');
...
const wrapReload = (contents) => {
    ...
    .then(changes => {
        ...
        for (const key of changes) {
            ...
            if (!value) {
                changeDetails[key] = null;
            } else {
                changeDetails[key] = isSecret ? '***' : value;
            }
        }
    ...
    .catch((e) => {
        logError(e, {}, 'Failed to reload environment variables');
    });
}
